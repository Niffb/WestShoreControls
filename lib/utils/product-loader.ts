...
/**
 * Load products for a specific category from JSON files
 */
export async function loadProductsForCategory(categorySlug: string): Promise<Product[]> {
  ...
  if (!directories) {
    console.warn(`No directory mapping found for category: ${categorySlug}`)
    return []
  }
  ...
  for (const directory of directories) {
    ...
    await loadJsonFilesRecursively(categoryPath, allProducts, categorySlug)
  }
